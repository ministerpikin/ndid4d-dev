<?php
/**
 * project_settings Class
 *
 * @used in  				project_settings Function
 * @created  				09:11 | 09-Apr-2019
 * @database table name   	project_settings
 */
	
	class cProject_settings extends cCustomer_call_log{
		...
		public $table_name = 'project_settings';
		
		public $default_reference = '';
		
		public $label = 'Project Settings';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/project_settings.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/project_settings.json' ), true );
		...
		protected function __before_save_changes(){
			if( ! ( isset( $_POST[ "id" ] ) && $_POST[ "id" ] ) ){
				if( isset( $_POST[ $this->table_fields["key"] ] ) && $_POST[ $this->table_fields["key"] ] ){
					$_POST[ $this->table_fields["key"] ] = trim( strtoupper( $_POST[ $this->table_fields["key"] ] ) );
					
					$_POST["tmp"] = clean3( strtolower( $_POST[ $this->table_fields["key"] ] ) );
					$this->class_settings[ 'use_replace' ] = 1;
				}
			}
		}
		...
		protected function _new_popup_form2(){
			...
			case 'edit_popup_form':
				...
				if( ! get_hyella_development_mode() ){
					$this->class_settings[ "form_display_not_editable_value" ][ $this->table_fields["key"] ] = 1;
					...
					$this->class_settings[ "form_display_not_editable_value" ][ $this->table_fields["comment"] ] = 1;
				}
				//
		...
		public $basic_data = array(
			'access_to_crud' => 1,
			'exclude_from_crud' => array( 'delete' => 1, 'create_new_record' => 1 ),
		);
	}
