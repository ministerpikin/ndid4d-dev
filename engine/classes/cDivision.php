<?php
	 ...
	 * division Class
	 *
	 * @used in  				division Function
	 * @created  				13:27 | 05-01-2013
	 * @database table name   	division
	 */

	/*
	|--------------------------------------------------------------------------
	| division Function in Settings Module
	|--------------------------------------------------------------------------
	|
	...
	*/
	...
	class cDivision{
		...
		private $table_fields = array(
			'name' => 'division001',
			'description' => 'division002',
			'head' => 'division003',
			'assistant' => 'division004',
			'secretary' => 'division005',
		);
		...
		function division(){
			...
			break;
			case 'get_division':
				$returned_value = $this->_get_division();
			break;
			}
		...
		private function _display_data_table(){
			...
			if($sql_result && is_array($sql_result)){
				...
				$err->class_that_triggered_error = 'cdivision.php';
		...
		private function _get_division(){
			...
			$all_division = execute_sql_query($query_settings);
			
			$division = array();
			
			if( is_array( $all_division ) && ! empty( $all_division ) ){
				
				foreach( $all_division as $category ){
					$division[ 'all' ][ $category[ 'id' ] ] = $category['name'];
					$division[ $category[ 'id' ] ] = $category;
				}
				
				//Cache Settings
				$settings = array(
					'cache_key' => $cache_key,
					'cache_values' => $division,
					'permanent' => true,
				);
