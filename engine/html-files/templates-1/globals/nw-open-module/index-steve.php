...
<style type="text/css">
	...
	<div class="row">
		<div class="col-md-12">
			...
			<?php if( isset( $data[ 'show_full_screen' ] ) && $data[ 'show_full_screen' ] ){
					...
					if( ! empty( $tabs ) ){ 
						foreach( $tabs as $ktbs => $tbs ){
							...
							if( isset( $tbs[ 'html' ] ) && $tbs[ 'html' ] ){
								...
								$hh1 .= '<div class="'.$d_active2.' tab-pane" id="tabd-'.$ktbs.'-' .  $unique_key . '">'. $tbs[ 'html' ] .'</div>';
...
function __perpare_buttons( $btns, $unique_key, $pid, $container ){
	...
	foreach( $btns as $kav => $av ){
		...
		$h .= '<li role="presentation"><a clickme="'. $kav .'" '.$ti.' role="menuitem" tabindex="-1" href="#" action="?action='.$av["action"].'&todo='.$av["todo"].'&html_replacement_selector='. $cont .'&container='. $container . $params2 .'" override-selected-record="'. $pid2 .'" '. $attrx .'class="custom-single-selected-record-button '. $class .'" unique_key="'. $unique_key .'">'. $add.$av["title"] .'</a></li>';
