<?php
	...
	// echo '<pre>'; print_r( strrpos( $e[ 'status' ], '.', 2 ) ); echo '</pre>';
		...
		if( ! empty( $data[ 'general_tasks3' ] ) && is_array( $data[ 'general_tasks3' ] ) ){
			if( $has )$h .= '<li class="divider"></li>';
			$h .= __perpare_buttons( $data[ 'general_tasks3' ], $unique_key, $pid, $container );
		}
			...
			if( isset( $agd[ 'title' ] ) && ! empty( $agd[ 'actions' ] ) ){
				...
				foreach( $agd[ 'actions' ] as $kav => $av ){
					if( isset( $av[ 'action' ] ) && $av[ 'action' ] && isset( $av[ 'todo' ] ) && $av[ 'todo' ] ){
						...
						$h .= '<li role="presentation"><a clickme='. $kav .' '.$ti.' role="menuitem" tabindex="-1" href="#" action="?action='.$av["action"].'&todo='.$av["todo"].'&html_replacement_selector=general-activity-window-'. $unique_key .'&container='. $container . $params2 .'" override-selected-record="'. $xid .'" class="skip-processing custom-single-selected-record-button '. $clasx .'" '. $attrx .' unique_key="'. $unique_key .'">'. $av["title"] .'</a></li>';
					}
				}
	...
	if( ! empty( $data[ 'general_actions' ] ) && is_array( $data[ 'general_actions' ] ) ){
		...
		foreach( $data[ 'general_actions' ] as $kav => $av ){
			...
			$b .= '<li role="presentation"><a clickme='. $kav .' role="menuitem" tabindex="-1" href="#" action="?action='.$av["action"].'&todo='.$av["todo"].'&container='. $container . $axt .'&table='. $tb . ( isset( $av[ 'params' ] ) ? $av[ 'params' ] : '' ) .'" '. $attrx .' override-selected-record="'. $xid .'" class="skip-processing custom-single-selected-record-button">'. $av["title"] .'</a></li>';
		}
