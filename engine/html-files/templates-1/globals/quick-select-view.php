	...
	if( isset( $data[ 'table_filter' ][ 'form_action' ] ) && isset( $data[ 'table_filter' ][ 'fields' ] ) && ! empty( $data[ 'table_filter' ][ 'fields' ] ) ){
		...
		foreach( $data[ 'table_filter' ][ 'fields' ] as $f ){
			...
			case 'select2':
				...
				$h .= '&nbsp;&nbsp;<input class="form-controlx '. ( isset( $f["class"] )?$f["class"]:'' ) .'" type="text" name="'. $f["name"] .'" value="'. $f["value"] .'" label="'. ( isset( $f["label"] )?$f["label"]:'' ) .'" placeholder="'. ( isset( $f["label"] )?$f["label"]:'' ) .'" action="'. $action .'" data-params="'. $f["data-params"] .'" minlength="'. $f["minlength"] .'" tags="'. $f["tags"] .'" />';
			break;
			case 'select':
		...
		?>
		<form action="<?php echo $data[ 'table_filter' ][ 'form_action' ]; ?>" method="post" class="activate-ajax skip_show_headers" id="datatable-filter">
		<?php echo $h; ?>
		</form>
		<script type="text/javascript">
