<?php 
	...
	$auto_sel = isset( $data[ 'spreadsheet' ][ 'auto_sel' ] ) ? $data[ 'spreadsheet' ][ 'auto_sel' ] : array();
	...
	$_htrsl = []; preg_match("/\&html_replacement_selector=\w+/", $action, $_htrsl); if( !$_htrsl ){ $action .= '&html_replacement_selector='. $container;}
	...
	$hide_form_btn = isset( $data[ 'hide_form_btn' ] ) ? $data[ 'hide_form_btn' ] : 0;
...
<?php
	if( ! empty( $data_object ) && ! empty( $columns ) && ! empty( $colHeaders ) ){
		...
		$h2 = '<form id="'. $handsontb_container .'-form" class="activate-ajax '.$cn_p.'" method="post" action="'. $xact . '">';
			...
			$h2 .= '<textarea name="data" class="hyella-data" style="display:none;"></textarea>';
		$h2 .= '</form>';
...
<script type="text/javascript" class="auto-remove">
	...
	nwBulkDataCapture.auto_sel = <?php echo ! empty( $auto_sel ) ? json_encode( $auto_sel ) : '{}'; ?>;
