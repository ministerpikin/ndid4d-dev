				...
				if( isset( $data["event_items"] ) && is_array( $data["event_items"] ) && ! empty( $data["event_items"] ) ){
					foreach( $data["event_items"] as $items ){
						...
						if( isset( $items["data"] ) && $items["data"] ){
							$idata = json_decode( $items["data"], true );
							if( isset( $idata["term4"] ) && $idata["term4"] == "production-order" ){
								$porder = 1;
								foreach( $idata as $ik1 => $iv1 ){
									$items[ $ik1 ] = $iv1;
								}
							}
						}
				  ...
				  <table class="table table-striped table-bordered table-hover" style="margin-top:5px;">
					 ...
					 <tbody>
						...
						foreach( $html1 as $ktitle => $sval ){
							...
							echo '<tr><td><strong>SUB TOTAL</strong></td><td align="right"><strong>'.$sval["quantity"].'</strong></td><td></td><td></td><td align="right"><strong>'.number_format( $sval["length"], 2 ).'</strong></td><td></td><td align="right"><strong>'.number_format( $sval["total"], 2 ).'</strong></td></tr>';
