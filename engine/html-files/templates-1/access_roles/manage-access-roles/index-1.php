...
<div class="portlet-title">
	<div class="caption">
		<a href="#" class="btn btn-default btn-xs custom-single-selected-record-button" action="?module=&action=access_roles&todo=manage_access_roles<?php echo $params; ?>" override-selected-record="<?php echo isset( $data["event"]["id"] )?$data["event"]["id"]:'1'; ?>" mod="1" title="Manage Access Roles"><i class="icon-refresh"></i></a>
	...
	<div class="col-md-10 col-md-offset-1">
		...
		<form id="access_role-form" action="?action=access_roles&todo=save_manage_access_role2<?php echo $params; ?>" class="activate-ajax" method="post">
			...
			<?php 
				...
				$apm = array();
			?>
			<textarea name="data" class="form-control hyella-data" style1="display:none;"><?php echo json_encode( $dsel ); ?></textarea>
		...
		<form id="client-form" >
			<?php
				...
				if( isset( $data["frontend_menus"] ) && ! empty( $data["frontend_menus"] ) ){
					foreach( $data["frontend_menus"] as $bk => $sval ){
						...
						if( isset( $sval["sub_menu"] ) && is_array( $sval["sub_menu"] ) && ! empty( $sval["sub_menu"] ) ){
							...
							$sval["module_name"] = ( isset( $sval['prefix'] )?$sval['prefix']:'' ) . $sval['title'];
							//$sys_modules[ $sval["module_name"] ]["data"][ $sval['title'] ][ $sval["id"] ] = $sval;
							
							foreach( $sval["sub_menu"] as $bk2 ){
								if( isset( $data["frontend_menus_function"][ $bk2 ]["title"] ) ){
									...
									if( isset( $sval2["sub_menu"] ) && is_array( $sval2["sub_menu"] ) && ! empty( $sval2["sub_menu"] ) ){
										...
										foreach( $sval2["sub_menu"] as $bk3 ){
											if( isset( $data["frontend_menus_function"][ $bk3 ]["title"] ) ){
												$sval3 = $data["frontend_menus_function"][ $bk3 ];
												
												$sval3["id"] = $bk3;
												$sval3["function_name"] = ( isset( $sval3['prefix'] )?$sval3['prefix']:'' ) . $sval3['title'];
				...
				if( isset( $data["basic_crud"] ) && ! empty( $data["basic_crud"] ) ){
					...
					$basic_crud_methods = array( 'create_new_record' => 'Create', 'edit' => 'Edit', /*'display_all_records_full_view' => 'View',*/ 'delete' => 'Delete', 'export' => 'Export' );
					
					foreach( $data["basic_crud"] as $sval ){
						if( isset( $sval["real_table"] ) && $sval["real_table"] ){
							$sval["table_name"] = $sval["real_table"];
						}
						if( isset( $sval["plugin"] ) && $sval["plugin"] ){
							$sval["table_name"] = $sval["plugin"] . '.' . $sval["table_name"];
						}
						...
						if( isset( $sval["add_to_crud"] ) && is_array( $sval["add_to_crud"] ) && ! empty( $sval["add_to_crud"] ) ){
							foreach( $sval["add_to_crud"] as $act2k => $act2 ){
								if( isset( $act2["title"] ) && isset( $act2["todo"] ) ){
									
									$act_val = array();
									$act_val["id"] = $sval["table_name"] . '.' . $act2k;
									$act_val["function_name"] = $act2["title"];
							...
							foreach( $sval["more_actions"] as $act2k => $act2 ){
								...
								if( isset( $act2["data"] ) && ! empty( $act2["data"] ) ){
									foreach( $act2["data"] as $act3 ){
										if( ! empty( $act3 ) ){
											foreach( $act3 as $bk1 => $bv1 ){
												...
												$act_val["function_name"] = isset( $bv1["text"] )?$bv1["text"]:$bk1;
									...
									if( isset( $act2["todo"] ) && ! isset( $sval["exclude_from_crud"][ $act2k ] ) ){
										...
										$act_val = array();
										$act_val["id"] = $sval["table_name"] . '.' . $act2k;
										$act_val["function_name"] = $act2["text"];
				...
				if( isset( $data["status_table"] ) && ! empty( $data["status_table"] ) ){
					...
					foreach( $data["status_table"] as $k1 => $sv ){
...
<script type="text/javascript" class="auto-remove">
	<?php 
		function ___format_access_capabilities( $val = array(), $selected, $dsel ){
			$serial = 0;
