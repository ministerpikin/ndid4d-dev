...
$html1 .= '<form class="activate-ajax list" method="POST" action="' . $faction . $params .'" id="form-'.$form_id.'">';
				...
				$h2 .= '<div class="col-md-'.$count.'">';
					...
					if( isset( $v[ 'fields' ] ) && $v[ 'fields' ] ){
						foreach( $v[ 'fields' ] as $f_key => $f_val ){
							...
							$d1 = __get_value( '', '', array( 'form_fields' => $ips, 'globals' => $gbal, 'mobile_class3' => $mobile_class3, 'mobile_class2' => $mobile_class2, 'mobile_class1' => $mobile_class1, 'mobile_class5' => $mobile_class5 ) );
...
if( isset( $data["form_script"] ) && $data["form_script"] ){
	//$preview = 1;
	if( $preview ){
		$html .= '<pre>var $formid = "form-'.$form_id.'"; ' . rawurldecode( $data["form_script"] ) . '</pre>';
	}else{
		$html .= '<script style1="display:none;" class="form-script">var $formid = "form-'.$form_id.'"; ' . rawurldecode( $data["form_script"] ) . ' </script>';
	}
}
