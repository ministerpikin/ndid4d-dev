 ...
 * wards Class
 *
 * @used in  				wards Function
 * @created  				Bay4 Mike | 02:36 | 21-Sep-2023
 * @database table name   	wards
 */
	
	class cWards extends cCustomer_call_log{
		...
		public $table_name = 'wards';
		
		public $default_reference = '';
		
		public $label = 'Ward List';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/wards.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/wards.json' ), true );
		...
		if( file_exists( dirname( __FILE__ ).'/dependencies/wards.json' ) ){
			...
			if( isset( $return[ "labels" ] ) ){
				...
				if( isset( $return[ "fields" ] ) ){
					...
					$return[ "labels" ][ $key ][ 'class' ] = ' selected-state selected-country select2 ';
					...
					$return[ "labels" ][ $key ][ 'form_field' ] = 'calculated';
					$return[ "labels" ][ $key ][ 'attributes' ] = ' action="?action=nwp_locations&todo=execute&nwp_action=states&nwp_todo=get_select2&source=wards" data-params=".selected-country" minlength="0" ';
					$return[ "labels" ][ $key ][ 'class' ] = ' selected-state select2 ';
					...
					$return[ "labels" ][ $key ][ 'form_field' ] = 'calculated';
					$return[ "labels" ][ $key ][ 'attributes' ] = ' action="?action=nwp_locations&todo=execute&nwp_action=lgas&nwp_todo=get_select2&source=wards" data-params=".selected-state" minlength="0" ';
