 ...
 * states Class
 *
 * @used in  				states Function
 * @created  				11:24 | 11-Sep-2019
 * @database table name   	states
 */
	
	class cStates extends cBase_class{
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/states.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/states.json' ), true );
		...
		function states(){
			...
			break;
			case 'get_states':
				$returned_value = $this->_get_states();
			break;
		...
		protected function _get_select2(){
			...
			$this->class_settings[ 'get_select2_option_type' ] = 'states';
		...
		protected function _display_all_records_full_view2(){
			...
			/*$this->class_settings[ 'filter' ][ "select" ] = " `". $this->table_name ."`.* , COUNT( `lgas`.`id` ) as 'lga_count' ";
			$this->class_settings[ 'filter' ][ "join" ] = " LEFT JOIN `nassco`.`lgas` ON `lgas`.`lgas50017` = `". $this->table_name ."`.`id` ";
			$this->class_settings[ 'filter' ][ "group" ] = " GROUP BY `". $this->table_name ."`.`id` ";*/

			// $this->class_settings[ 'filter' ][ "select" ] = " `". $this->table_name ."`.* , (SELECT COUNT( `lgas`.`id` ) FROM `nassco`.`lgas` ON `lgas`.`lgas50017` = `". $this->table_name ."`.`id` ) as 'lga_count' ";
		...
		private function _get_states(){
			...
			if( isset( $this->class_settings['country'] ) && $this->class_settings['country'] ){
				...
				$all_states = execute_sql_query($query_settings);
				
				$states = array();
				$states_geo = array();
				
				if( is_array( $all_states ) && ! empty( $all_states ) ){
					
					foreach( $all_states as $category ){
						$states[ $category['id'] ] = $category;
						if( $category['geo_ip_state'] )$states_geo[ strtolower($category['geo_ip_state']) ] = $category;
					}
		...
		if( file_exists( dirname( __FILE__ ).'/dependencies/states.json' ) ){
			...
			if( isset( $return[ "labels" ] ) ){
				...
				if( isset( $return[ "fields" ] ) ){
					...
					$key = $return[ "fields" ][ "name" ];
					$return[ "labels" ][ $key ][ 'field_label' ] = 'Name of State';

					
					$key = $return[ "fields" ][ "geo_zone" ];
					$return[ "labels" ][ $key ][ 'calculations' ] = array(
						'type' => 'record-details',
						// 'type' => 'local-govt-count',
