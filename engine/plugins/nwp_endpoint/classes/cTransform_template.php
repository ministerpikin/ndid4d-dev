<?php
/**
 * transform_template Class
 *
 * @used in  				transform_template Function
 * @created  				Hyella Nathan | 16:55 | 25-Jul-2023
 * @database table name   	transform_template
 */
	
	class cTransform_template extends cCustomer_call_log{
		...
		public $table_name = 'transform_template';
		
		public $default_reference = '';
		
		public $label = 'Transformation Template';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/transform_template.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/transform_template.json' ), true );
		...
		public function _csv_to_json( array $o, array $opt ){
			$r = [];
			if( $o ){
				if( $opt ){
					$headers = [];
					$header_key = isset($opt['header_key']) && $opt['header_key'] ? $opt['header_key'] : '';
					if( $header_key ){
						$header_key = explode(',', $header_key);
						foreach ($header_key as $key) {
							if( !$headers ){
								$headers = isset($o[ $key ]) && $o[ $key ] ? $o[ $key ] : $headers;
							}else{
								$headers = isset( $headers[ $key ] ) && $headers[ $key ] ? $headers[ $key ] : $headers;
							}
						}
					}					

					if( isset($opt['data_key']) && $opt['data_key'] ){
						$data_key = array_map('trim', explode(',', $opt['data_key']));
						foreach ($data_key as $key) {
							$o = isset( $o[ $key ] ) && $o[ $key ] ? $o[ $key ] : $o;
						}
					}

					$header_row = isset( $opt['header_row']) ? (int)$opt['header_row'] : '';
					$start_row = isset( $opt['start_row']) ? (int)$opt['start_row'] : '';
					$end_row = isset( $opt['end_row']) ? (int)$opt['end_row'] : '';
				
					if( !$headers ){
						$headers = isset($o[ $header_row ]) && $o[ $header_row ] ? $o[ $header_row ] : [];
					}

					// if( isset( $data ) ){
						switch (1) {
							case ($start_row !== '') && ($end_row !== ''):
								$o = array_slice($o, $start_row, $end_row);
							break;
							case  $start_row !== '' :
								$o = array_slice($o, $start_row);
							break;
							case $end_row !== '':
								$o = array_slice($o, 0, $end_row);
							break;
						}

						foreach ($o as $sval) {
							if( $headers && $sval ){
								if( count( $headers ) == count( $sval ) ){
									$r[] = array_combine( $headers, $sval );
								}
							}
						}
					// }
		...
		public function _json_to_json( array $o, array $opt ){
			$r = [];
			if( $o ){
				if( $opt ){
					if( isset( $opt['error_key'] ) && $opt['error_key'] ){
						$err = 0;
						$error_key = array_map('trim', explode(',', $opt['error_key']));
