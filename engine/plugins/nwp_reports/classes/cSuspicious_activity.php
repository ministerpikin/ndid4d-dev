<?php
/**
 * suspicious_activity Class
 *
 * @used in  				suspicious_activity Function
 * @created  				Hyella Nathan | 10:54 | 30-Aug-2023
 * @database table name   	suspicious_activity
 */
	
	class cSuspicious_activity extends cCustomer_call_log{
		...
		public $table_name = 'suspicious_activity';
		
		public $default_reference = '';
		
		public $label = 'Suspicious Enrolment Activity';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/suspicious_activity.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/suspicious_activity.json' ), true );
		...
		if( file_exists( dirname( __FILE__ ).'/dependencies/suspicious_activity.json' ) ){
			...
			if( isset( $return[ "labels" ] ) ){
				if( isset( $return[ "fields" ] ) && $return['fields'] ){
					...
					$return[ "labels" ][ $key ][ 'attributes' ] = ' action="?action=nwp_reports&todo=execute&nwp_action=suspicious_activity&nwp_todo=get_es_select2&field=device" minlength="0" ';
