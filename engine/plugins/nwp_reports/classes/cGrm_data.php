 ...
 * grm_data Class
 *
 * @used in  				grm_data Function
 * @created  				Hyella Nathan | 12:48 | 15-Aug-2023
 * @database table name   	grm_data
 */
	
	class cGrm_data extends cCustomer_call_log{
		...
		public $table_name = 'grm_data';
		
		public $default_reference = '';
		
		public $label = 'Grievance & Redress Mechanism';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/grm_data.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/grm_data.json' ), true );
	...
	function grm_data(){
		if( file_exists( dirname( __FILE__ ).'/dependencies/grm_data.json' ) ){
			...
			if( isset( $return[ "labels" ] ) ){
				...
				$return[ "labels" ][ $key ][ 'form_field' ] = 'calculated';
				$return[ "labels" ][ $key ][ 'attributes' ] = ' action="?action=users&todo=get_select2" minlength="0" ';
				$return[ "labels" ][ $key ][ 'class' ] = ' select2 ';
