<style> 
	...
	.form-control-limit{ padding:2px;margin-left:5px;line-height:1.5; width: 70px; }
	.chart-axis{ padding: .4rem; line-height: 1.5; font-weight: 400; }
</style>
<div class="row" id="build-query">
	   	...
	   	<div id="search-form" class="search-mode-form" <?php echo $aform == 'search-form' ? '' : ' style="display:none;" '; ?>>
			<form id="create-query-form" class="client-form">
				...
				<div>
					...
					<select class="form-control" name="condition" type="select" required="required" onchange="nwSearch.changeCondition( '', '', 'create-query-form' );">
					</select>
				</div>
				<br />
	...
	<div class="col-md-8">
		...
		<form class="activate-ajax" action="<?php echo $action; ?>" id="query-execute-form" style="text-align:center;">
			...
			<hr />
			<?php
				if( isset( $data['show_display_limit'] ) && $data['show_display_limit'] ){ ?>
					<div id="">
		   				<label data-bs-toggle="popover" data-bs-content="Total Number of Entries to be displayed on the chart entries on the Bar Chart" data-bs-toggle="hover">Display Limit : &nbsp;<span class="text-large"><i class="icon-info-sign"></i></span>&nbsp; <input type="checkbox" name="use_limit" id="use_limit" onchange="nwSearch.changeLimit('', '', 'query-execute-form')" /></label>
		   				<input type="number" <?php if( isset( $dd['report_limit'] ) && $dd['report_limit'] ){ echo 'value="'. intval( $dd['report_limit'] ) .'"'; } ?> name="query_limit" class="hidden form-control-limit" />
		   			</div>
		   			<br />
				<?php }

				if( isset( $data['show_chart_options'] ) && $data['show_chart_options'] ){ ?>
					<div id="chart_options" style="display:none;">
						<div class="row">
							<div class="col-md-12">
								<p data-bs-toggle="popover" data-bs-content="Choose which fields would be used to display grouped report entries on the Bar Chart" data-bs-toggle="hover"><b>Report Axis:</b> &nbsp;&nbsp;<span class="text-large"><i class="icon-info-sign"></i></span></p>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6">
				   				<label>Grouped Field on X - Axis:&nbsp;</label>
				   				<select name="x-axis" class="chart-axis"></select>
				   			</div>						
							<div class="col-md-6">
				   				<label>Grouped Field on Y - Axis:&nbsp; </label>
				   				<select name="y-axis" class="chart-axis"></select>
				   			</div>
						</div>
					</div>
		   			<br />
			...
			<?php  if( isset( $show_save_options ) && ! empty( $show_save_options ) ){ ?>
				<?php foreach( $show_save_options as $s1 => $s2 ){ ?>
					<label class="radio-inlinex" style="margin-right:15px;">
						 <input required type="radio" id="radio-<?php echo $s1; ?>" name="save_options" value="<?php echo $s1; ?>" onchange=""> <small><?php echo $s2; ?></small>
					</label>
				<?php } ?>
			<br>
			<?php } ?>
