<?php
/**
 * enrolment_center Class
 *
 * @used in  				enrolment_center Function
 * @created  				Hyella Nathan | 12:32 | 15-Aug-2023
 * @database table name   	enrolment_center
 */
	
	class cEnrolment_center extends cCustomer_call_log{
		...
		public $table_name = 'enrolment_center';
		
		public $default_reference = '';
		
		public $label = 'Enrolment Center';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/enrolment_center.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/enrolment_center.json' ), true );
		...
		function enrolment_center(){
			...
			break;
			case 'get_select2':
				$fn = '_get_default_select2';
				// print_r($this->plugin_instance->plugin_classes_config);exit;
		...
		protected function _display_all_records_full_view2(){
			...
			case "display_all_records_frontend":
				...
				if( $join_from && $id ){
					...
					$join = " JOIN `". $this->class_settings[ 'database_name' ] ."`.`". $ed ."` ON `". $ed ."`.`". $al[ $ed ]->table_fields[ 'center' ] ."` = `". $this->table_name ."`.`id` AND `". $ed ."`.`record_status` = '1' AND `". $ed ."`.`". $al[ $ed ]->table_fields[ $key ] ."` = '". $id ."' ";

					$this->class_settings['prefix_title'] = get_name_of_referenced_record( array( 'id' => $id, 'table' => 'enrolment_' . $key ) ) . ' - ';

					$group = " GROUP BY `". $ed ."`.`". $al[ $ed ]->table_fields[ 'center' ] ."` ";
