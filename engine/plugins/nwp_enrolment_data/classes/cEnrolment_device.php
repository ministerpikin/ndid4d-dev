<?php
/**
 * enrolment_device Class
 *
 * @used in  				enrolment_device Function
 * @created  				Hyella Nathan | 14:47 | 16-Aug-2023
 * @database table name   	enrolment_device
 */
// Turning Point for Mike
	
	class cEnrolment_device extends cCustomer_call_log{
		...
		public $table_name = 'enrolment_device';
		
		public $default_reference = '';
		
		public $label = 'Enrolment Device';
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/enrolment_device.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/enrolment_device.json' ), true );
		...
		protected function _generate_new_data_capture_form2(){
			...
			$this->class_settings[ 'hidden_records' ][ $this->table_fields[ 'country' ] ] =  1;
			$this->class_settings[ 'hidden_records' ][ $this->table_fields[ 'state' ] ] =  1;
			$this->class_settings[ 'hidden_records' ][ $this->table_fields[ 'lga' ] ] =  1;
		...
		protected function _display_all_records_full_view2(){
			...
			case "display_all_records_others":
				...
				if( $join_from && $id ){
					...
					$join = " JOIN `". $this->class_settings[ 'database_name' ] ."`.`". $ed ."` ON `". $ed ."`.`". $al[ $ed ]->table_fields[ 'device' ] ."` = `". $this->table_name ."`.`id` AND `". $ed ."`.`record_status` = '1' AND `". $ed ."`.`". $al[ $ed ]->table_fields[ $key ] ."` = '". $id ."' ";

					$this->class_settings['prefix_title'] = get_name_of_referenced_record( array( 'id' => $id, 'table' => 'enrolment_' . $key ) ) . ' - ';

					$group = " GROUP BY `". $ed ."`.`". $al[ $ed ]->table_fields[ 'device' ] ."` ";
