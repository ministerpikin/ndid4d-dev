<?php
/**
 * enrolment_demographic Class
 *
 * @used in  				enrolment_demographic Function
 * @created  				Hyella Nathan | 20:09 | 16-Aug-2023
 * @database table name   	enrolment_demographic
 */
	
	class cEnrolment_demographic extends cCustomer_call_log{
		...
		public $table_name = 'enrolment_demographic';
		
		public $default_reference = '';
		
		public $label = 'Enrolment Demographic Heartbeat';
		
		private $associated_cache_keys = array(
			'enrolment_demographic',
		...
		function __construct(){
			...
			if( file_exists( dirname( __FILE__ ).'/dependencies/enrolment_demographic.json' ) ){
				$return = json_decode( file_get_contents( dirname( __FILE__ ).'/dependencies/enrolment_demographic.json' ), true );
		...
		function enrolment_demographic(){
		...
		protected function _display_all_records_full_view2(){
			$where = '';
			$disable_btn = isset( $_GET[ 'disable_btn' ] ) ? $_GET[ 'disable_btn' ] : 0;
			$enrol_id = isset( $_GET[ 'enrol_id' ] ) ? $_GET[ 'enrol_id' ] : 0;
			$spilt_screen = 0;
			$action_to_perform = $this->class_settings[ 'action_to_perform' ];
			$show_form = 0;
			
			if( $enrol_id ){
				$where = " AND `". $this->table_name ."`.`". $this->table_fields[ 'enrol_id' ] ."` = '". $enrol_id ."' ";
			}
