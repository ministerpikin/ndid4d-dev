        ...
        if($skip_create == false && self::$checked['path'] == false) {
            if(!file_exists(self::$path."/".self::$securityKey."/") || !is_writable(self::$path."/".self::$securityKey."/")) {
                ...
                if(!file_exists(self::$path."/".self::$securityKey."/")) {
                    @mkdir(self::$path."/".self::$securityKey."/", $perm );
                }
                if(!is_writable(self::$path."/".self::$securityKey."/")) {
                    @chmod(self::$path."/".self::$securityKey."/", $perm );
                }
                if(!file_exists(self::$path."/".self::$securityKey."/") || !is_writable(self::$path."/".self::$securityKey."/")) {
    ...
    public static function autoconfig($name = "") {
        ...
        if(self::$storage == "" || self::$storage == "auto") {
            ...
            }elseif(extension_loaded('xcache'))
            {
                ...
                $result_of_sql_queryeconfig = false;
                ...
                if (file_exists(self::getPath()."/config.".$os['unique'].".cache.ini"))
                {
                    ...
                    if(!isset($info['value'])) {
                        $result_of_sql_queryeconfig = true;
                ...
                if(!file_exists(self::getPath()."/config.".$os['unique'].".cache.ini") || $result_of_sql_queryeconfig == true) {
...
~
    ...
    public static function deleteMulti($object = array()) {
        ...
        foreach($object as $driver=>$name)  {
            ...
            $result_of_sql_queryes[$n] = self::delete($name);
        }
        return $result_of_sql_queryes;
    ...
    public static function setMulti($mname = array(), $time_in_second_for_all = 600, $skip_for_all = false) {
        ...
        foreach($mname as $object){
            ...
            if($name!="" && $value!="") {
                $result_of_sql_queryes[$n] = self::set($name,$value, $time, $skip);
            }
            // echo "<br> ----- <br>";
    ...
    public static function getMulti($object = array()) {
        ...
        foreach($object as $driver=>$name)  {
            ...
            $result_of_sql_queryes[$n] = self::get($name);
        }
        return $result_of_sql_queryes;
    ...
    private static function files_set($name,$value,$time_in_second = 600, $skip_if_existing = false) {
        ...
        $database_name= self::selectDB($name);
        $name = $database_name['item'];
        $folder = $database_name['db'];
        ...
        if($write == true ) {
            try {
				file_put_contents($file, self::encode($value,$time_in_second) );
				$perm = 0666;
    ...
    private static function getMemoryName($name) {
        ...
        if(self::$sys['method'] == "memcache" || $database_name['db'] == "memcache") {
            self::memcache_addserver();
        }elseif(self::$sys['method'] == "memcached" || $database_name['db'] == "memcached") {
    ...
    private static function xcache_increment($name, $step = 1) {
        $orgi = $name;
        $name = self::getMemoryName($name);
        $result_of_sql_queryet =xcache_inc($name, $step);
        if($result_of_sql_queryet === false) {
            self::xcache_set($orgi,$step,3600);
            return $step;
        } else {
            return $result_of_sql_queryet;
        }
    }
    ...
    private static function xcache_decrement($name, $step = 1) {
        $orgi = $name;
        $name = self::getMemoryName($name);
        $result_of_sql_queryet = xcache_dec($name, $step);
        if($result_of_sql_queryet === false) {
            self::xcache_set($orgi,$step,3600);
            return $step;
        } else {
    ...
    private static function apc_increment($name, $step = 1) {
        $orgi = $name;
        $name = self::getMemoryName($name);
        $result_of_sql_queryet = apc_inc($name, $step, $fail);
        if($result_of_sql_queryet === false) {
    ...
    private static function apc_decrement($name, $step = 1) {
        $orgi = $name;
        $name = self::getMemoryName($name);
        $result_of_sql_queryet = apc_dec($name, $step, $fail);
        if($result_of_sql_queryet === false) {
            self::apc_set($orgi,$step,3600);
            return $step;
        } else {
    ...
    private static function pdo_exist($name) {
        ...
        $x = self::db(array('db'=>$database_name['db']))->prepare("SELECT COUNT(*) as `total` FROM ".self::$table." WHERE `name`=:name");
    ...
    private static function pdo_stats($full = false) {
        ...
        if($full == true) {
            ...
            $stm->execute();
            $result_of_sql_queryesult = $stm->fetchAll();
            $result_of_sql_queryes['data'] = $result_of_sql_queryesult;
        }
        $stm = self::db()->prepare("SELECT COUNT(*) as `total` FROM ".self::$table."");
        $stm->execute();
        $result_of_sql_queryesult = $stm->fetch();
        $result_of_sql_queryes['record'] = $result_of_sql_queryesult['total'];
        if(self::$path!="memory") {
            $result_of_sql_queryes['size'] = filesize(self::getPath()."/".self::$filename);
        }
    ...
    private static function selectDB($object) {
        ...
        if(is_array($object)) {
            ...
            $key = $key[0];
            $result_of_sql_queryes['db'] = $key;
            $result_of_sql_queryes['item'] = self::safename($object[$key]);
        } else {
            $result_of_sql_queryes['item'] = self::safename($object);
        }

        if($result_of_sql_queryes['db'] == "" && self::$sys['method'] == "files") {
            $result_of_sql_queryes['db'] = "files";
        }

        // for auto database
        if($result_of_sql_queryes['db'] == "" && self::$storage== "mpdo") {
            $create_table = false;
            ...
            $database_name= self::$autodb->prepare("SELECT * FROM `db` WHERE `item`=:item");
            $database_name->execute(array(
                ":item" => $result_of_sql_queryes['item'],
            ));
            $row = $database_name->fetch(PDO::FETCH_ASSOC);
            if(isset($row['dbname'])) {
                // found key
                $result_of_sql_queryes['db'] = $row['dbname'];
            } else {
                ...
                // get last key
                $database_name= self::$autodb->prepare("SELECT * FROM `db` ORDER BY `id` DESC");
                $database_name->execute();
                $row = $database_name->fetch(PDO::FETCH_ASSOC);
                $database_namename = isset($row['dbname']) ? $row['dbname'] : 1;
                $fsize = file_exists(self::getPath()."/".$database_namename.".cache") ? filesize(self::getPath()."/".$database_namename.".cache") : 0;
                if($fsize > (1024*1024*(Int)self::$autosize)) {
                    $database_namename = (Int)$database_namename + 1;
                }
                try {
                    $insert = self::$autodb->prepare("INSERT INTO `db` (`item`,`dbname`) VALUES(:item,:dbname)");
                    $insert->execute(array(
                        ":item" => $result_of_sql_queryes['item'],
                        ":dbname"   => $database_namename
                    ));
                } catch (PDOexception  $e) {
                    die('Database Error - Check A look at self::$autodb->prepare("INSERT INTO ');
                }

                $result_of_sql_queryes['db'] = $database_namename;
    ...
    private static function pdo_get($name) {
        $database_name= self::selectDB($name);
        $name = $database_name['item'];
        // array('db'=>$database_name['db'])

        // self::startDebug($database_name,"",__LINE__,__FUNCTION__);

        $stm = self::db(array('db'=>$database_name['db']))->prepare("SELECT * FROM ".self::$table." WHERE `name`='".$name."'");
        $stm->execute();
        $result_of_sql_queryes = $stm->fetch(PDO::FETCH_ASSOC);

        if(!isset($result_of_sql_queryes['value'])) {
            return null;
        } elseif((Int)$result_of_sql_queryes['added'] + (Int)$result_of_sql_queryes['endin'] <= (Int)@date("U")) {
            return null;
        } else {
            // decode value on SQL;
            $data = self::decode($result_of_sql_queryes['value']);
    ...
    private static function pdo_decrement($name, $step = 1) {
        ...
        $int = self::get($name);
        try {
            $stm = self::db(array('db'=>$database_name['db']))->prepare("UPDATE ".self::$table." SET `value`=:new WHERE `name`=:name ");
            $stm->execute(array(
    ...
    private static function pdo_delete($name) {
        ...
        return self::db(array('db'=>$database_name['db']))->exec("DELETE FROM ".self::$table." WHERE `name`='".$name."'");
    }
    ...
    private static function pdo_set($name,$value,$time_in_second = 600, $skip_if_existing = false) {
        ...
        if($skip_if_existing == true) {
            try {
                $insert = self::db(array('db'=>$database_name['db']))->prepare("INSERT OR IGNORE INTO ".self::$table." (name,value,added,endin) VALUES(:name,:value,:added,:endin)");
                try {
            ...
            try {
                $insert = self::db(array('db'=>$database_name['db']))->prepare("INSERT OR REPLACE INTO ".self::$table." (name,value,added,endin) VALUES(:name,:value,:added,:endin)");
                try {
                    $value = self::encode($value);
                } catch(Exception $e) {
                    die("Sorry! phpFastCache don't allow this type of value - Name: ".$name);
    ...
    private static function db($option = array()) {
        $vacuum = false;
        $database_namename = isset($option['db']) ? $option['db'] : "";
        $database_namename = $database_namename != "" ? $database_namename : self::$filename;
        if($database_namename!=self::$filename) {
            $database_namename = $database_namename.".cache";
        }
        // debuging
        ...
        if(self::$storage == "pdo") {
            ...
            if(self::$objects['pdo']=="") {
                ...
                if(!file_exists(self::getPath()."/".$database_namename)) {
                    $initDB = true;
                } else {
                    if(!is_writable(self::getPath()."/".$database_namename)) {
						$perm = 0777;
						if( defined("NWP_DIR_PERM") ){
							$perm = NWP_DIR_PERM;
						}
                        @chmod(self::getPath()."/".$database_namename, $perm );
                        if(!is_writable(self::getPath()."/".$database_namename)) {
                            die("Please CHMOD 0777 or any Writable Permission for ".self::getPath()."/".$database_namename);
                        }
                    }
                }
                ...
                try {
                    self::$objects['pdo'] = new PDO("sqlite:".self::getPath()."/".$database_namename);
                    ...
                    $time = filemtime(self::getPath()."/".$database_namename);
                    ...
                    die("Can't connect to caching file ".self::getPath()."/".$database_namename);
                }
            ...
            // start self PDO
            if(!isset(self::$multiPDO[$database_namename])) {
                //  self::$objects['pdo'] == new PDO("sqlite:".self::$path."/cachedb.sqlite");
                if(self::$path!="memory") {
                    if(!file_exists(self::getPath()."/".$database_namename)) {
                        $initDB = true;
                    ...
                    try {
                        self::$multiPDO[$database_namename] = new PDO("sqlite:".self::getPath()."/".$database_namename);
                        self::$multiPDO[$database_namename]->setAttribute(PDO::ATTR_ERRMODE,
                            PDO::ERRMODE_EXCEPTION);

                        if($initDB == true) {
                            self::initDatabase(self::$multiPDO[$database_namename]);
                        }
                        ...
                        if($vacuum == true) {
                            if(!isset($option['skip_clean'])) {
                                self::$multiPDO[$database_namename]->exec("DELETE FROM ".self::$table." WHERE (`added` + `endin`) < ".@date("U"));
                            }
                            self::$multiPDO[$database_namename]->exec('VACUUM');
                        }
                ...
                return self::$multiPDO[$database_namename];
