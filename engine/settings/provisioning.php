<?php
	...
	if( isset( $sys_env[ 'NWP_OPEN_PMAIL' ] ) ){
	...
	if( isset( $sys_env[ 'INSTALL_PATH' ] ) && $sys_env[ 'INSTALL_PATH' ] ){
		...
		// define( 'HYELLA_INSTALL_PATH' , 'ndid4d-dev-main/engine/' );
	...
	if( isset( $sys_env[ 'HYELLA_INSTALL_ENGINE' ] ) && $sys_env[ 'HYELLA_INSTALL_ENGINE' ] ){
		...
		// define( 'HYELLA_INSTALL_ENGINE' , 'ndid4d-dev-main/' );
	...
	if( isset( $sys_env[ 'HYELLA_FRONTEND_URL' ] ) && $sys_env[ 'HYELLA_FRONTEND_URL' ] ){
		define( 'HYELLA_FRONTEND_URL', $sys_env[ 'HYELLA_FRONTEND_URL' ]  );
	}else{
		define( 'HYELLA_FRONTEND_URL', 'http://localhost/ndid4d-dev/m-and-e/main.php' );
	}
	...
	if( isset( $sys_env[ 'HTTPS' ] ) && $sys_env[ 'HTTPS' ] ){
	...
	if( isset( $sys_env[ 'ES_VERSION' ] ) ){
		...
		// define( "ES_VERSION", 'C:\xampp\htdocs\ndid4d-dev-main\engine\plugins\nwp_orm\elasticsearch-php-main\vendor\autoload.php' );
	...
	if( isset( $sys_env[ 'ES_PASSWORD' ] ) && $sys_env[ 'ES_PASSWORD' ] ){
		...
		// define( "ES_PASSWORD", 'Vr_2*eRB0azG1KACIg6b' );
		// define( "ES_PASSWORD", 'ZJqRGmAcNdrHMWWM87CSccA2tUXmPRwvwbXJaRKJrQcDgs' );
	...
	if( isset( $sys_env[ 'ES_HOST' ] ) && $sys_env[ 'ES_HOST' ] ){
		...
		// define( "ELASTIC_HOST", 'http://me.maybeachtech.com.ng:9200' );
	}
	...
	if( isset( $sys_env[ 'NWP_V3_ENDPOINT' ] ) && $sys_env[ 'NWP_V3_ENDPOINT' ] ){
		...
		// define( "NWP_V3_ENDPOINT", 'http://localhost/ndid4d-dev-main/engine/api/' );
	...
	if( isset( $sys_env[ 'MYSQL_BULK_LOAD_FILE_PATH' ] ) ){
		...
		// define( 'MYSQL_BULK_LOAD_FILE_PATH' , 'C:\xampp\htdocs\ndid4d-dev-main\engine\tmp\filescache\endpoint_log\data.txt' );
	...
	if( isset( $sys_env[ 'ENDPOINT_PAGINATION_SLEEP' ] ) && ( $sys_env[ 'ENDPOINT_PAGINATION_SLEEP' ] ) ){
		define( 'ENDPOINT_PAGINATION_SLEEP' , $sys_env[ 'ENDPOINT_PAGINATION_SLEEP' ]  );
	}else{
		define( 'ENDPOINT_PAGINATION_SLEEP' , 5); //secs
	}

	if( isset( $sys_env[ 'ENDPOINT_CALL_MAX_MEMORY' ] ) && ( $sys_env[ 'ENDPOINT_CALL_MAX_MEMORY' ] ) ){
		define( 'ENDPOINT_CALL_MAX_MEMORY' , $sys_env[ 'ENDPOINT_CALL_MAX_MEMORY' ]  );
	}else{
		define( 'ENDPOINT_CALL_MAX_MEMORY', 32); //IN MB;
	}

	if( isset( $sys_env[ 'ENDPOINT_CALL_MAX_EXECUTION_TIME' ] ) && ( $sys_env[ 'ENDPOINT_CALL_MAX_EXECUTION_TIME' ] ) ){
		define( 'ENDPOINT_CALL_MAX_EXECUTION_TIME' , $sys_env[ 'ENDPOINT_CALL_MAX_EXECUTION_TIME' ]  );
	}else{
		define( 'ENDPOINT_CALL_MAX_EXECUTION_TIME', 200); //IN secs;
	}
	
	if( isset( $sys_env[ 'ENDPOINT_CALL_REPORT_MAIL' ] ) && ( $sys_env[ 'ENDPOINT_CALL_REPORT_MAIL' ] ) ){
		define( 'ENDPOINT_CALL_REPORT_MAIL' , $sys_env[ 'ENDPOINT_CALL_REPORT_MAIL' ]  );
	}else{
		define( 'ENDPOINT_CALL_REPORT_MAIL', 1);
	}
	
	if( isset( $sys_env[ 'ENDPOINT_LOG_MAIL_REPORT_FREQUENCY' ] ) && ( $sys_env[ 'ENDPOINT_LOG_MAIL_REPORT_FREQUENCY' ] ) ){
		define( 'ENDPOINT_LOG_MAIL_REPORT_FREQUENCY' , $sys_env[ 'ENDPOINT_LOG_MAIL_REPORT_FREQUENCY' ]  );
	}else{
		define( 'ENDPOINT_LOG_MAIL_REPORT_FREQUENCY', 24);
	}

	if( isset( $sys_env[ 'RABBITMQ_HOST' ] ) && ( $sys_env[ 'RABBITMQ_HOST' ] ) ){
		define( 'RABBITMQ_HOST' , $sys_env[ 'RABBITMQ_HOST' ]  );
	}else{
		define( 'RABBITMQ_HOST' , 'localhost');
	}

	if( isset( $sys_env[ 'RABBITMQ_PORT' ] ) && ( $sys_env[ 'RABBITMQ_PORT' ] ) ){
		define( 'RABBITMQ_PORT' , $sys_env[ 'RABBITMQ_PORT' ]  );
	}else{
		define( 'RABBITMQ_PORT' , 5672);
	}

	if( isset( $sys_env[ 'RABBITMQ_WEB_HOST' ] ) && ( $sys_env[ 'RABBITMQ_WEB_HOST' ] ) ){
		define( 'RABBITMQ_WEB_HOST' , $sys_env[ 'RABBITMQ_WEB_HOST' ]  );
	}else{
		define( 'RABBITMQ_WEB_HOST' , 'localhost');
	}

	if( isset( $sys_env[ 'RABBITMQ_WEB_PORT' ] ) && ( $sys_env[ 'RABBITMQ_WEB_PORT' ] ) ){
		define( 'RABBITMQ_WEB_PORT' , $sys_env[ 'RABBITMQ_WEB_PORT' ]  );
	}else{
		define( 'RABBITMQ_WEB_PORT' , 15672);
	}

	if( isset( $sys_env[ 'KIBANA_HOST' ] ) && ( $sys_env[ 'KIBANA_HOST' ] ) ){
		define( 'KIBANA_HOST' , $sys_env[ 'KIBANA_HOST' ]  );
	}else{
		define( 'KIBANA_HOST' , 'localhost');
	}

	if( isset( $sys_env[ 'KIBANA_PORT' ] ) && ( $sys_env[ 'KIBANA_PORT' ] ) ){
		define( 'KIBANA_PORT' , $sys_env[ 'KIBANA_PORT' ]  );
	}else{
		define( 'KIBANA_PORT' , 5601);
	}

	if( isset( $sys_env[ 'NIN_VERIFICATION_ON_ACCOUNT_CREATION' ] ) ){
		define( 'NIN_VERIFICATION_ON_ACCOUNT_CREATION' , $sys_env[ 'NIN_VERIFICATION_ON_ACCOUNT_CREATION' ]  );
	}else{
		define( 'NIN_VERIFICATION_ON_ACCOUNT_CREATION' , 1);
	}

	if( isset( $sys_env[ 'NVS_SERVER_URL' ] ) && ( $sys_env[ 'NVS_SERVER_URL' ] ) ){
		define("NVS_SERVER_URL", $sys_env["NVS_SERVER_URL"]);
	}

	if( isset( $sys_env[ 'NVS_SERVER_USERNAME' ] ) && ( $sys_env[ 'NVS_SERVER_USERNAME' ] ) ){
		define("NVS_SERVER_USERNAME", $sys_env["NVS_SERVER_USERNAME"]);
	}

	if( isset( $sys_env[ 'NVS_SERVER_PASSWORD' ] ) && ( $sys_env[ 'NVS_SERVER_PASSWORD' ] ) ){
		define("NVS_SERVER_PASSWORD", $sys_env["NVS_SERVER_PASSWORD"]);
	}

	if( isset( $sys_env[ 'NVS_SERVER_ORGID' ] ) && ( $sys_env[ 'NVS_SERVER_ORGID' ] ) ){
		define("NVS_SERVER_ORGID", $sys_env["NVS_SERVER_ORGID"]);
	}

	if( isset( $sys_env[ 'NWP_HIDE_EXPORT_SIGNATORY_FIELDS' ] ) && ( $sys_env[ 'NWP_HIDE_EXPORT_SIGNATORY_FIELDS' ] ) ){
		define("HIDE_EXPORT_SIGNATORY_FIELDS", $sys_env["NWP_HIDE_EXPORT_SIGNATORY_FIELDS"]);
	}

	if( isset( $sys_env[ 'NWP_AUDIT_TRAIL_RETENTION_CYCLE' ] ) ){
		define("AUDIT_TRAIL_RETENTION_CYCLE", $sys_env["NWP_AUDIT_TRAIL_RETENTION_CYCLE"]); //in hours
	}else{
		define("AUDIT_TRAIL_RETENTION_CYCLE", 0.08);
	}

	if( isset( $sys_env[ 'NWP_ELASTIC_SEARCH_PAGINATION_DEFAULT_LIMIT' ] ) && ( $sys_env[ 'NWP_ELASTIC_SEARCH_PAGINATION_DEFAULT_LIMIT' ] ) ){
		define("ELASTIC_SEARCH_PAGINATION_DEFAULT_LIMIT", $sys_env["NWP_ELASTIC_SEARCH_PAGINATION_DEFAULT_LIMIT"]); //in hours
	}else{
		define("ELASTIC_SEARCH_PAGINATION_DEFAULT_LIMIT", 10000);
	}

	if( isset( $sys_env[ 'NWP_ELASTIC_DELAY_FOR_CACHE_REFRESH' ] ) && ( $sys_env[ 'NWP_ELASTIC_DELAY_FOR_CACHE_REFRESH' ] ) ){
		define("ELASTIC_DELAY_FOR_CACHE_REFRESH", $sys_env["NWP_ELASTIC_DELAY_FOR_CACHE_REFRESH"]);
	}

	if( isset( $sys_env[ 'NWP_IGNORE_ENDPOINT_DATE_FILTERS' ] ) ){
		define("IGNORE_ENDPOINT_DATE_FILTERS", $sys_env["NWP_IGNORE_ENDPOINT_DATE_FILTERS"]);
	}else{
		//define("IGNORE_ENDPOINT_DATE_FILTERS", 1);
	}

	if( isset( $sys_env[ 'NWP_USE_SESSION_FOR_REPORT_FILTER_CACHE' ] ) ){
		define("USE_SESSION_FOR_REPORT_FILTER_CACHE", $sys_env["NWP_USE_SESSION_FOR_REPORT_FILTER_CACHE"]);
	}else{
		define("USE_SESSION_FOR_REPORT_FILTER_CACHE", 1);
	}

	$package_config["plugins_loaded"]['nwp_reports'] = 'cNwp_reports';
	$package_config["plugins_loaded"]['nwp_device_management'] = 'cNwp_device_management';
	$package_config["plugins_loaded"]['nwp_enrolment_data'] = 'cNwp_enrolment_data';

	$package_config["exclude_plugin_classes"]["nwp_device_management"] = array(
		'frequent_changes' => 1,
	...
	$package_config["exclude_plugin_classes"]["nwp_enrolment_data"] = array(
		'enrolment_device' => 1,
	...
	$package_config["plugin_classes_config"]["nwp_enrolment_data"] = array(
		'enrolment_center' => [ 'use_es' => 1 ],
		'enrolment_partner' => [ 'use_es' => 1 ],
		'enrolment_agent' => [ 'use_es' => 1 ]
	);
